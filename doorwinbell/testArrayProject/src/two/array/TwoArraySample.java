...
//		 int[][] tar = { { 10, 20, 30 }, { 11, 22, 33 }, { 15, 25, 35 } }; // 3행 3열 정변 배열
//		int[] tar[] = { { 10, 20, 30 }, { 11, 22, 33 }, { 15, 25, 35 } }; // 3행 3열 정변 배열
		int tar[][] = { { 10, 20, 30 }, { 11, 22, 33 }, { 15, 25, 35 } }; // 3행 3열 정변 배열
		...
		for (int i = 0; i < tar.length; i++) {
			System.out.println(i + "행이 참조하는 1차원 배열의 값갯수 : " + tar[i].length + "열"); // 각행별 열갯수(1차원배열 방갯수)
		}
	...
	public void testTwoArray2() {
		...
		System.out.println("tar.lengrh : " + tar.length + "행");
	...
	public void testTwoArray4() {
		...
		int[][] tar = new int[3][];
		// 위의 선언은 heap메모리에 주소 기록할 레퍼런스 3개를 배열로 할당함
