...
public class StringSample {
	...
	public void immutableString() {
		...
		// 문자열 저장소는 같은 문자열을 두 번 기록하지 않는다. (두번째 기록시에는 이미 기록된 문자열의 주소 리턴)
		// 문자열 저장소에 기록된 문자열값은 수정 불가능(immutable)
		// 기록된 문자열 값을 수정하면, cpu가 문자열을 읽어가서 수정 연산처리를 하고,
		// 변경된 문자열을 문자열 저장소에 새로 할당 기록함 => 메모히 사용량이 늘어나므로, 수정 작업 부적합함

		// 자바가 문자열을 취급하도록 String 클래스를 제공함
		// String 사용 방식을 기본자료형처럼 쓸 수 있게 제공함
		// 기본적으로 클래스 사용 방식은 클래스명 레퍼런스변수 = new 생성자([초기값, 초기값, ...]);
		// 기본자료형 사용 방식 : 자료형 변수명 = 값;
		String str = "test string"; // jvm이 literal pool에 자동 할당하고 주소를 리턴함
	...
	public void usingStringBuilder() {
		// 문자열 값의 조작(추가, 삭제, 변경) 작업이 필요하면, String을 StringBuilder | StringBuffer로 바꿈
		// 별도의 메모리 공간을 만들어서 문자열을 기록함 => 문자열 조작 (기존 문자열값을 변경함)
		// => String으로 바꿈
		...
		// StringBuilder에 기록된 문자열을 char[]에 복사해 넣을 경우
		...
		System.out.println("sb가 참조하는 문자열의 s 문자의 순번 : " + sb.indexOf("s"));
		System.out.println("sb가 참조하는 문자열의 ing 문자의 순번 : " + sb.indexOf("ing"));
		...
		System.out.println("6번 인덱스부터 끝글자까지 추출 str : " + str);

		str = sb.substring(0, 5);
		System.out.println("0번 인덱스부터 5번 인덱스 앞 글자까지 추출 : " + str);

		// StringBuilder ==> String으로 바꿀 때
		str = sb.toString();
		System.out.println("String : " + str);
	}

	public void usingSplit() {
		// String s = "java linux oracle cloud jdbc html5 css3 javaScript jQuery jsp
		// servlet";
		// String[] star = s.split(" ");
