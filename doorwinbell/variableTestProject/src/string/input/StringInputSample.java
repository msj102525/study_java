...
public class StringInputSample {
	// java.util.Scanner의 next() 메소드 테스트
	...
	// java.util.Scanner의 nextLine() 메소드 테스트
	...
	// java.util.Scanner의 nextLine() 메소드 사용시 주의사항
	public void testNextLine2() {
		Scanner sc = new Scanner(System.in);
		// 연속으로 여러 종류의 값을 입력 받을 때
		// nextLine()은 이전 입력값 뒤에 붙은 엔터키를 값으로 읽어드림 (문제점)
		...
		// 입력버퍼에서 엔터키를 읽어내서 버리는 작업
		
		sc.nextLine(); // 이전에 버퍼에 남아있는 nextLine의 enter값을 빼버림
