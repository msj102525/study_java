...
public class EmployeeMenu {
	...
	private static EmployeeController econtroll = new EmployeeController();
	...
	public static void displayMenu() {
		do {
			...
			switch(no) {
			case 1:	//ArrayList<Employee> list = econtroll.selectList();	
					//printList(list);  //리스트 출력용
					printList(econtroll.selectList());
					break;
			case 2:	//String empId = inputEmpId();
					//Employee emp = econtroll.selectOne(empId);
					//System.out.println("\n조회된 직원 정보는 다음과 같습니다.");
					//System.out.println(emp);
					try {
						System.out.println(econtroll.selectOne(inputEmpId()).toString());
						//null.메소드() 형태로 실행될 때, NullPointerException 발생함
						//null.toString() 으로 실행시키면, NullPointerException 이용할 수 있음
					}catch(NullPointerException e) {
						System.out.println("\n해당 사번에 대한 직원정보가 존재하지 않습니다.");
					}
					break;
			case 3:	if(econtroll.insertEmployee(inputEmployee()) > 0) {
						System.out.println("\n새로운 직원 정보가 등록되었습니다.");
					}else {
						System.out.println("\n새로운 직원 정보 등록에 실패했습니다. 관리자에게 문의하세요.");
					}
					break;
			case 4:	if(econtroll.updateEmployee(modifyEmployee()) > 0) {
						System.out.println("\n직원 정보가 성공적으로 수정되었습니다.");
					}else {
						System.out.println("\n직원 정보 수정에 실패했습니다. 관리자에게 문의하세요.");
					}
					break;
			case 5:	if(econtroll.deleteEmployee(inputEmpId()) > 0) {
			...
			case 7:	printList(econtroll.selectJob(inputJobId()));
					break;
			case 8:	printList(econtroll.selectName(inputEmpName()));
					break;
	...
	private static Employee modifyEmployee() {
		...
		//급여, 보너스포인트, 직급코드, 부서코드 수정 처리를 위해 입력받아 emp 에 저장함
		System.out.print("급여 : ");
